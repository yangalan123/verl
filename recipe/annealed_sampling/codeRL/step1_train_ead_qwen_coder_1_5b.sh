#!/bin/bash
# Exploratory Annealed Decoding (EAD) GRPO on Qwen2.5-Coder-1.5B-Instruct.
# Mirrors the math recipe `annealed_sampling_minimal_rl_negexp.sh` but on the
# Eurus-2 code subset with stdin/stdout evaluation via prime_code +
# HumanEval+ via the new local humanevalplus reward.
#
# Usage:
#   bash recipe/annealed_sampling/codeRL/step1_train_ead_qwen_coder_1_5b.sh
#
# Hyperparameters mirror the small-model defaults reported in the paper:
#   tau_max = 1.2, tau_min = 0.1, d_0 = 200, alpha = 5, d_max = 40000.
...
# EAD schedule (same names as the math recipe so logs line up).
decay_freq=200
start_temp=1.2
end_temp=0.1
warmup_period=10
