...
def annealed_sampling_processor(token_ids: Union[list[int], tuple[int]], logits: torch.Tensor, 
                               ...
                               decay_freq: int = 50) -> torch.Tensor:
    """
    ...
    """
    # Calculate the current temperature based on sequence length
    current_temp = stability_temp + (exploration_temp - stability_temp) * np.exp(-len(token_ids) / decay_freq)
