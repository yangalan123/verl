...
\paragraph{Exploratory Annealed Decoding.} 
...
\vspace{-0.2in}
    ...
    \caption{
     The annealing schedule with different decay rates $d$. A larger $d$ slows the cooling, front-loading exploration over more tokens. We set $c=10, \tau_{\mathrm{max}}=1.2, \tau_{\mathrm{min}}=0.1$ for illustration.
        % \zz{Nit: before cutoff, should we use a gray-ish color to (1) indicate no temp schedule is applied; (2) different $d$ are the same before cutoff?}
    }
...
\end{wrapfigure}
%
% As illustrated in \cref{fig: annealing_schedule_demo}, a larger $d$ slows down the temperature cooling rate and thus encourages exploration of alternative generations for more early-position tokens (``front-load'' exploration), while a smaller $d$ does the opposite. 
%
...
%
% We set the temperature $\tau=1.0$ for this part to avoid intervening generation of this part.
%
We fix the temperature at $\tau = 1.0$ in this part to avoid interfering with the generation process.
%
...
\paragraph{Ensuring Stability with Truncated  Importance Sampling.}
...
% \paragraph{Truncated Importance Sampling Correction.}
...
% \paragraph{The Dynamic Temperature Schedule.}
...
% \paragraph{Ensuring Stability with Importance Sampling.}
