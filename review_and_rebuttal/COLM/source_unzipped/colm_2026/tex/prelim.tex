...
% \end{equation}
% \end{small}
\shortparagraph{Reinforcement Learning with Verifiable Rewards (RLVR).}
...
\begin{small}
\begin{align*}
&J_{\text{DAPO}}(\theta)=\E_{\substack{x \sim \mathcal{D},\\y^{(1:G)} \overset{iid}{\sim} \pi_{\theta_{\text{old}}}(\cdot\mid x)}} 
\left[\frac{1}{\sum_{i=1}^G |y^{(i)}|}
\sum_{i=1}^G\sum_{t=1}^{|y^{(i)}|}\min\left\{r^{(i)}_{t}(\theta)A_i,\mathrm{clip}\left(r^{(i)}_{t}(\theta),1-\varepsilon_{\text{low}},1+\varepsilon_{\text{high}}\right)A_i\right\}
\right]\\
...
\shortparagraph{Pass@$\bm k$.} 
Pass@$k$ measures the probability that at least one of $k$ independent outputs from a language model is correct. 
%
Let $p_x$ denote the per-prompt accuracy of the model $\pi$. The pass@$k$ accuracy is defined as $\E_{x\sim\mathcal{D}}\left[1-(1-p_x)^k\right].$
%
High diversity benefits this metric, as a model with diverse outputs is more likely to maintain non-negligible $p_x$ across prompts.
%

\shortparagraph{Entropy.} 
