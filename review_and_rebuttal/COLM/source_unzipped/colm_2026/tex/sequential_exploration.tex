...
% % \vspace{-0.1in}
%     \centering
%     \includegraphics[width=0.45\linewidth]{iclr2026/figures/branching_factor_figure/piecewise_ebf_Llama-3-8B-Instruct_mean_seq_entropy_fixed_start.pdf}
%     % \vspace{-0.3in}
%     \caption{
...
% \begin{figure}[t]
%     \centering
% \includegraphics[width=0.45\linewidth]{figures/branching_factor_figure/bon_from_middle_demo_v2.pdf}
% % \vspace{-0.05in}
%     \caption{
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/branching_factor_figure/piecewise_ebf_Llama-3-8B-Instruct_mean_seq_entropy_fixed_start.pdf}
        \caption{}
        \label{fig: entropy_sequential_dynamic}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/branching_factor_figure/bon_from_middle_demo_v2.pdf}
        \caption{}
        \label{fig:forking_experiment}
    \end{subfigure}
    \vspace{-5pt}
    \caption{
        (a) Avg. entropy shrinks with output positions for Llama-3-8B-Instruct on MMLU dataset;
        (b) A ``forking'' experiment on DeepSeek-Llama-3 shows early branching (high-entropy region) yields higher Maj@$k$ on MMLU than late branching (low-entropy region).
    }
    \label{fig:combined_branching_analysis}
    \vspace{-10pt}
\end{figure}
...
%
We reference the Data Processing Inequality (DPI)~\citep{shannon1948mathematical} as a \textit{loose theoretical motivation} to investigate these entropy dynamics.\colmedit{ Strictly speaking, the DPI bounds entropy of a Markov chain $X{\to}Y{\to}Z$ and does not directly govern the position-wise conditional entropy of a fixed-policy autoregressive model. We invoke it only as an analogy: it suggests, but does not prove, that adding more context (more conditioning tokens) cannot increase the average uncertainty about the next token. The actual claim we rely on is the empirical entropy decay in \cref{fig: entropy_sequential_dynamic} and the forking experiment in \cref{fig:forking_experiment}, not a formal information-theoretic guarantee.} While not a strict derivation for language  models, the DPI provides a framework to hypothesize that expected conditional entropy tends to decrease as the context grows:\footnote{While specific rollouts may have late high-entropy positions, the probability of this is exponentially small with position $t$~\citep{yang2025alignment}, making the overall trend a reliable heuristic.}
% \vspace{-10pt}
\vspace{-5pt}
\begin{small}
\begin{equation}
...
% \begin{figure}[t]
% \centering
% \includegraphics[width=0.55\linewidth]{figures/branching_factor_figure/piecewise_ebf_Llama-3-8B-Instruct_mean_seq_entropy_fixed_start.pdf}
% \caption{Average entropy shrinks with output positions for Llama-3-8B-Instruct on MMLU dataset.}
% \label{fig: entropy_sequential_dynamic}
% \vspace{-0.15in}
% \end{figure}
...
%
Aligning our strategy with the natural dynamics of generation, we arrive at a simple yet powerful design principle: \textbf{explore early and exploit late}. We emphasize that this is a practical heuristic grounded in the empirical entropy dynamics of autoregressive models, not a claim that late-stage exploration is never useful. Individual sequences may contain critical late-stage decisions; however, in expectation, the marginal value of exploration is highest at the beginning. Moreover, \alg does not eliminate late-stage stochasticity entirely---the floor temperature $\tau_{\min} > 0$ ensures that a non-trivial degree of randomness is retained throughout generation.
