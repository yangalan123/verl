...
\begin{small}
\begin{align*}
&J_{\text{DAPO}}(\theta)=\E_{x \sim \mathcal{D},y^{(1:G)} \overset{iid}{\sim} \pi_{\theta_{\text{old}}}(\cdot\mid x)} 
\left[\frac{1}{\sum_{i=1}^G |y^{(i)}|}\times\right.\\
&\left.\sum_{i=1}^G\sum_{t=1}^{|y^{(i)}|}\min\left\{r^{(i)}_{t}(\theta)A_i,\mathrm{clip}\left(r^{(i)}_{t}(\theta),1-\varepsilon_{\text{low}},1+\varepsilon_{\text{high}}\right)A_i\right\}
\right]\\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.55\linewidth]{icml2026/figures/branching_factor_figure/piecewise_ebf_Llama-3-8B-Instruct_mean_seq_entropy_fixed_start.pdf}
...
%
Let $p_x$ denote the underlying accuracy of the language model $\pi$ given one prompt $x$. The pass@$k$ accuracy is defined as $\E_{x\sim\mathcal{D}}\left[1-(1-p_x)^k\right].$
%
