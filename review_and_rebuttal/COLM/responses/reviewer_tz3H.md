...
* **Status (rebuttal timeline).** Within the rebuttal window we report a short-horizon version of the sweep. We note one subtlety we will be explicit about in the paper: the decay rate grows as `d_s = min(d_0 + alpha*s, d_max)`, so a large `d_max` only takes effect after enough training steps; the full multi-order-of-magnitude sweep therefore requires a longer-horizon run, which is in progress. We will update this thread and the camera-ready with the converged sweep.
* **Expected message.** Our existing data already shows that performance is essentially flat for any `d_max >= 200`; the only failure mode is `d_max = 25 = d_0`, which collapses the schedule to a fixed-temperature run and removes the global-step adaptation. The extended sweep is designed to confirm that no upper-side failure mode exists either, and the short-horizon results so far are consistent with this.
